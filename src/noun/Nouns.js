...
export default function Nouns() {
    ...
    useEffect(() => {
        if (appUser !== 'logout' && appUser !== null) {
            getAllData(`nouns/${appUser.id}`, setBoth, setShowLoader);
        }
        if (appUser === 'logout') {
            navigate('/');
        }
    ...
    function tableAction(type, obj) {
        ...
        if (type === 'del') {
            if (window.confirm('Do You Want to Delete The Verb.')) {
                setShowLoader(true);
                deleteData(obj.id, `nouns/${appUser.id}`, setBoth, setShowLoader);
            }
        }
        if (type === 'cpt') {
            if (window.confirm(`Do You Want to ${obj.sts === 'cpt' ? 'InComplete' : 'Complete'} The Question`)) {
                ...
                setShowLoader(true);
                updateData(newObj, obj.id, `nouns/${appUser.id}`, setBoth, setShowLoader);
            }
        }
    }
    ...
    function addUpdate(type, obj) {
        ...
        if (type === 'Add') {
            obj.id = uniqueId();
            obj.sts = 'inCpt';
            InsertData(obj, `nouns/${appUser.id}`, setBoth, setShowLoader);
        }
        if (type === 'Update') {
            setEditable(false);
            updateData(obj, obj.id, `nouns/${appUser.id}`, setBoth, setShowLoader);
        }
    }
    function setClose(t) {
        if (t === 'edit') {
